...
26.01 	- (Сделано) импортировать основную логику - бывший main.py, переименовать его в carDetector
	...
	- (Сделано)сделать кнопку помощь и справку
	
	- (Сделано)настройки детектора в firefox не сохраняются, в хроме сохраняются.  возможно при обновлении настроек надо блокировать основной поток как для случая обновления рамок. посмотреть.
	
	- (Сделано) 9.03 2 проблемы: кривой адрес хаба, не сохраняются настройки детектора. Работаем с настройками. 
		выключаем передачу постов на хаб, временно, а то заоб спамить в консоль. потом не забыть включить стр 357app метка ctrl 6
	- (Сделано) проблема - данные на концентратор отсылаются на один и тот-же IP - IP не меняется при изменении его на сайте
	
	- (Под вопросом) сделать прореживание точек в детекторе движения. - должно улучшиться время цикла.- вопрос - так-ли это? проверить выключив временно подсчет среднего. 
		
	- (Сделано) сделать диагностику ответа от хаба на web странице при нажатии D.

	
	- (Решено)сделать другой фивикон
	- (Сделано) поправить стиль модальной таблицы всей - перейти на bootstrap?
	...
	- при нажатии редактировать/сохранить несколько раз без редактирования слетают рамки. на сервак вместо рамок посылается чушь. 
...
(Решено)Проблема: если нажать в web редактировать и сразу сохранить, не редактируя полигоны, грузятся тестовые рамки... при проверке не воспроизвелось....
